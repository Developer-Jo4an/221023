...
builder
  ...
  .createExtraReducer({
    thunkName: "load",
    saveData(state, {payload: {data: {areas, buildings, exchanger, account}}}) {
      ...
      state.settings = {
        areas,
        buildings: buildings /*partsBuildings(buildings)*/
      }
  ...
  .createExtraReducer({
    ...
    func: async function ({area, cell, type}) {
      const {data} = await build({area, cell, building_type: type});
